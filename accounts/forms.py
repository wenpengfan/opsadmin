...
from accounts.models import UserInfo, RoleList, PermissionList, DelegateList
...
class AddUserForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['username'].label = u'账号'
...
class EditUserForm(forms.ModelForm):
    class Meta:
        model = UserInfo
        fields = ('username', 'email', 'nickname', 'role', 'is_active', 'is_staff',)
        widgets = {
            ...
            'is_staff': forms.Select(choices=((True, u'是'),(False, u'否')),attrs={'class': 'form-control', 'style': 'width:500px;'}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['role'].label = u'角色'
        self.fields['is_active'].label = u'状态'
        self.fields['is_staff'].label = u'普通管理员'
...
class DelegateListForm(forms.ModelForm):
    class Meta:
        NAMES = AppOwner.objects.values_list("id", "name")

        model = DelegateList
        exclude = ("id", "delegater_dingding", "operator", "delegate_status", \
                   "delegate_from_id", "delegate_from_name", "delegate_to_id", "delegate_to_name",)
        widgets = {
            'delegate_from': Select(choices=(x for x in NAMES), attrs={'class': 'form-control','style': 'width:450px;'}),
            'delegate_to': Select(choices=(x for x in NAMES), attrs={'class': 'form-control','style': 'width:450px;'}),
            'starttime': DateTimeInput(attrs={'class': 'form-control', 'style': 'width:450px;',
                                      'placeholder': u'2020-01-01 12:00:00', 'autocomplete': 'off'}),
            'endtime': DateTimeInput(attrs={'class': 'form-control', 'style': 'width:450px;',
                                      'placeholder': u'2020-01-01 12:00:00', 'autocomplete': 'off'}),
        }
